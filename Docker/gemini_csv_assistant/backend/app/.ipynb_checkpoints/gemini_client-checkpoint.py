...
MODEL = os.environ.get("GEMINI_MODEL", "gemini-1.5")  # change via env if needed
...
def ask_gemini_for_code(prompt: dict) -> Dict[str, str]:
    ...
    Behavior:
    - Uses the filename and user query to build a small prompt.
    - Calls Gemini (official client if installed; otherwise REST request).
    - Attempts to extract a python code block. If not found, returns a safe default.
    """
    if not API_KEY:
    ...
    if _HAS_GENAI:
        try:
            # newer client supports genai.configure or genai.Client; handle both
            try:
            ...
            # resp may have a .text attribute or nested fields
            text = getattr(resp, "text", None) or str(resp)
        except Exception as e:
            text = f"Error calling genai client: {e}"
    else:
        # 2) Fallback: REST call to generativelanguage endpoint
        endpoint = f"https://generativelanguage.googleapis.com/v1beta/models/{MODEL}:generateContent"
        headers = {
            "Content-Type": "application/json",
            # Many setups accept Bearer with API key; if your key uses a different auth method change accordingly.
        ...
        try:
            ...
            r.raise_for_status()
            j = r.json()
            # Response shape can vary across SDK/REST; attempt likely fields
            if isinstance(j, dict):
                if "candidates" in j and len(j["candidates"]) > 0:
                    text = j["candidates"][0].get("content", "")
                elif "outputs" in j and len(j["outputs"]) > 0:
                    # sometimes outputs contains content
    ...
    if not code:
        # No code block — return a safe default snippet
        default = "# Default: return first 10 rows\noutput = df.head(10)\n"
        return {"pandas_code": default, "explanation": explanation}

    # Ensure the snippet defines `output`. If not, try to append a safe guard.
    if "output" not in code and "return" not in code:
        code = (
            ...
            "    output\n"
            "except NameError:\n"
            "    # If no explicit output, try to pick a plausible dataframe-like var\n"
